...
BigInteger &BigInteger::operator<<=(int q) {
...
void shift_right(const std::vector<uint32_t> &q, int w, std::vector<uint32_t> &res) {
    res.resize(std::max(0, (int) q.size() - w / 32));
    copy(q.begin() + w / 32, q.end(), res.begin());
    if (res.empty())
        res.push_back(0);
    else {
        uint32_t next = 0;
